...
public class CalendarTool {
	...
	/** 获取当前日历的年月 x为年，y为月 */
	public PenjinCalendarPoint getNowCalendar() {
		PenjinCalendarPoint p = new PenjinCalendarPoint(mYear, mMonth);
		return p;
	}
	...
	public List<DateEntity> getDateEntityList(int year, int month) {
		...
		int dayOfWeek = 0;// 得到当前年月的第一天为星期几
	...
	public PenjinDateEntityList getPenjinDateEntityList(int year ,int month)
	{
		mDataList.clear();
		int dayOfNowCalendar = 42;// 当前日历板的日期总数 7*6个数
		int dayOfWeek = 0;// 得到当前年月的第一天为星期几
		...
		int startIndex=previoursMonthDays;
		int endIndex=previoursMonthDays+days-1;
		return new PenjinDateEntityList(this.mDataList,startIndex,endIndex);
	}
